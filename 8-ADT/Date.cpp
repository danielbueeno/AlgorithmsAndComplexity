//NMEC: 96127
//NICK: Daniel Henrique Bueno Frank
...
Date* DateCreate(int yy, int mm, int dd) {
  ...
  Date* d = (Date*)malloc(sizeof(Date)); //allocate space for a block and return the address of that block
...
int DateCompare(const Date* a, const Date* b) {
  int diff = (int)a->year - (int)b->year;
  if(diff != 0) return diff;

  diff = (int)a->month - (int)b->month;
  if(diff != 0) return diff;

  diff = (int)a->day - (int)b->day;
 return diff;
}
